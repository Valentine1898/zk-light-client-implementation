...
use plonky2_test_case4_1::data_type::{Block, BlockType, Data, EpochBlock};
use plonky2_test_case4_1::sig_hash_circuit::{
    aggregation_two, hash_circuit_proof, sig_circuit_proof,
};
use plonky2_test_case4_1::sig_hash_circuit::{decode_hex, verification};
...
pub fn main() -> Result<()> {
        ...
        match height {
            ...
            _ => {
                match i {
                    ...
                    _ => epoch_id = blockchain[i - 3].get_hash().clone(),
                }
                prev_hash = blockchain[i - 1].get_hash();
    ...
    let (mut validators_circuit, mut validators_proof): (
        ...
        CircuitData::to_owned(None.unwrap()),
        ProofWithPublicInputs::to_owned(None.unwrap()),
    );
    ...
    // prove only epoch blocks
    for i in 0..blockchain.len() {
        if blockchain[i].get_height() == 1 {
            if i == 0 || i == 1 || i == 2 {
                epoch_block = const_block.clone();
            } else if (i % 2) == 1 {
                epoch_block = EpochBlock::new();
                epoch_block.validators_pk = blockchain[i - 3].get_v_pk().unwrap().clone();
            }
            ...
            (circuit_hash, proof_hash) = hash_circuit_proof(
                msg.as_bytes(),
                &decode_hex(&blockchain[i].get_hash())?,
                &mut sha256_circuits,
            );
            ...
            (circuit_sig, proof_sig) = sig_circuit_proof(
                ...
                epoch_block.validators_pk.get(&(i % 2)).unwrap().as_slice(),
                &mut ed25519_circuits,
            );
            println!("Proof signature sz: {}", proof_hash.to_bytes().len());
            ...
            println!(
                "Validators' proof signature sz: {}",
                validators_proof.to_bytes().len()
            );
            // add proofs for hashes of two previous epochs
            if i > 0 {
                (aggregated_circuit, aggregated_proof) = aggregation_two(
                    (&proofchain[i - 2].0 .0, &proofchain[i - 2].0 .1),
                    Some((&aggregated_circuit, &aggregated_proof)),
                )?;
                if i > 2 {
                    (aggregated_circuit, aggregated_proof) = aggregation_two(
                        (&proofchain[i - 4].0 .0, &proofchain[i - 4].0 .1),
                        Some((&aggregated_circuit, &aggregated_proof)),
                    )?;
                }
            }
    ...
    verification((&aggregated_circuit, &aggregated_proof))
}
